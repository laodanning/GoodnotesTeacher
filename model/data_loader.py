...
class Input_Datasets(Dataset):
    def __init__(self, train_dir=None, test_dir=None, data_seed=100, tokenizer = BertTokenizer.from_pretrained('hfl/chinese-lert-small')
, cache_dir='../datasets_cache', num_proc = 80, use_cache = True):
    ...
    def load_datasets(self):
        ...
        return load_dataset("csv", data_files=train_files, cache_dir=self.cache_dir ,**dataset_args, ), load_dataset("csv", data_files=test_files, cache_dir=self.cache_dir ,**dataset_args,delimiter='\t',)


    # 对数据进行tokenize，由于datasets可以进行multiprocess，故将此任务放到这。
    def tokenize_example(self, example):
        # 去除query30词以后的内容，超长query会在padding以后拖慢整个batch的计算速度
        query_encoding = self.tokenizer(example['sentence1'][:30],
                                    truncation=True, padding='max_length', max_length=400,
                                    return_token_type_ids=True)
        note_encoding = self.tokenizer(example['sentence2'],
                                    truncation=True, padding='max_length', max_length=400,
                                    return_token_type_ids=True)
        query_input_ids = query_encoding['input_ids']
        query_token_type_ids = query_encoding['token_type_ids']
        query_attention_mask = query_encoding['attention_mask']
        note_input_ids = note_encoding['input_ids']
        note_token_type_ids = note_encoding['token_type_ids']
        note_attention_mask = note_encoding['attention_mask']
        target = example['label']
        return {'input_ids': query_input_ids, 'token_type_ids': query_token_type_ids, 'attention_mask': query_attention_mask, 
                'note_input_ids': note_input_ids, 'note_token_type_ids': note_token_type_ids, 'note_attention_mask': note_attention_mask,
                'target': target}
...
class Distill_Datasets(Input_Datasets):
    def tokenize_example_train(self, example):
        ...
        if not isinstance(example['sentence1'], str) or not isinstance(example['sentence2'], str) or not isinstance(example['label'],float):
            print(example)
        ...
        query_encoding = self.tokenizer(example['sentence1'],
                                    truncation=True, padding='max_length', max_length=400)
        note_encoding = self.tokenizer(example['sentence2'],
                                    truncation=True, padding='max_length', max_length=400)
        query_input_ids = query_encoding['input_ids']
        ...
        note_attention_mask = note_encoding['attention_mask']
        target = example['label']
        return {'input_ids': query_input_ids, 'attention_mask': query_attention_mask, 
                'note_input_ids': note_input_ids, 'note_attention_mask': note_attention_mask,
                'target': target}
